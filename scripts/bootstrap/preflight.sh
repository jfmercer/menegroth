...
# ---- 1Password --------------------------------------------------------------
step "1Password — vault items readable (via the agent's service-account token)"
...
op_read_check() { # op_read_check <op-reference>
  if [[ -f "$token_file" ]]; then
    ...
    op read "$1" >/dev/null 2>&1
  fi
}
if command -v op >/dev/null 2>&1; then
  [[ -f "$token_file" ]] || warn "no service-account token at $token_file — testing with your op session instead"
