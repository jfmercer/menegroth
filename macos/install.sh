...
if [[ ! -f "$TOKEN_FILE" ]]; then
  echo "==> Storing the 1Password service-account token"
  echo "    Create it at 1password.com → Developer → Service Accounts, granting"
  echo "    READ-ONLY access to the '$OP_VAULT' vault ONLY, then paste the"
  echo "    token (it will not echo):"
  read -rs OP_TOKEN
  echo
  umask 177
  printf '%s\n' "$OP_TOKEN" > "$TOKEN_FILE"
...
# rest); only its public half leaves the vault.
if ! op_sa item get unlock-ssh-key --vault "$OP_VAULT" >/dev/null 2>&1; then
  echo "==> Generating the unlock SSH key inside 1Password"
  echo "    NOTE: this needs write access. If the service account is read-only"
  echo "    (recommended), create the item with YOUR account instead:"
  echo "      op item create --category 'SSH Key' --title unlock-ssh-key \\"
  echo "        --vault '$OP_VAULT' --ssh-generate-key ed25519"
